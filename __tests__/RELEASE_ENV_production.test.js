...
describe('rELEASE_ENV=production', () => {
  describe('isQA', () => {
    it('is false when RELEASE_ENV=production', () => {
      expect.assertions(1);
  ...
  describe('isReleaseEnvProd', () => {
    it('is true when RELEASE_ENV=production', () => {
      expect.assertions(1);
      expect(envVarHelpers.isReleaseEnvProd).toBe(true);
    });
  });
  describe('releaseEnvShort', () => {
    it("is 'prod' when RELEASE_ENV=production", () => {
      expect.assertions(1);
      expect(envVarHelpers.releaseEnvShort).toBe('prod');
    });
  });
  describe('releaseEnv', () => {
    it("is 'production' when RELEASE_ENV=production", () => {
      expect.assertions(1);
      expect(envVarHelpers.releaseEnv).toBe('production');
    });
  });
  describe('isDevNonReleaseEnv', () => {
    it('is always false when RELEASE_ENV=production', () => {
      expect.assertions(1);
