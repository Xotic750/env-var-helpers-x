...
describe('rELEASE_ENV=development', () => {
  describe('isQA', () => {
    it('is false when RELEASE_ENV=dev', () => {
      expect.assertions(1);
  ...
  describe('isReleaseEnvDev', () => {
    it('is true when RELEASE_ENV=dev', () => {
      expect.assertions(1);
  ...
  describe('releaseEnvShort', () => {
    it("is 'prod' when RELEASE_ENV=dev", () => {
      expect.assertions(1);
      expect(envVarHelpers.releaseEnvShort).toBe('dev');
    });
  });
  describe('releaseEnv', () => {
    it("is 'development' when RELEASE_ENV=dev", () => {
      expect.assertions(1);
      expect(envVarHelpers.releaseEnv).toBe('dev');
    });
  });
});
